...
TypesElement.prototype.addChild = function(child) {
    var schema = this.schemas[child.$targetNamespace];
    assert(child instanceof SchemaElement);    
    if (schema) {
        schema.children = schema.children.concat(child.children);
        schema.includes = schema.includes.concat(child.includes);
        extend(schema.xmlns, child.xmls);
        extend(schema.complexTypes, child.complexTypes);
        extend(schema.types, child.types);
        extend(schema.elements, child.elements);
    }
    else {
...
WSDL.prototype._processNextInclude = function(includes, callback) {
    ...
    open_wsdl(includePath, function(err, wsdl) {
        ...
        schema = self.definitions.schemas[include.namespace || wsdl.definitions.$targetNamespace];
        if (schema) {
          schema.includes = schema.includes.concat(wsdl.definitions.includes);
          extend(schema.xmlns, wsdl.definitions.xmls);
          extend(schema.complexTypes, wsdl.definitions.complexTypes);
          extend(schema.types, wsdl.definitions.types);
          extend(schema.elements, wsdl.definitions.elements);
        }
        else {
...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns, attributes) {
    var self = this,
        parts = [], subparts, attrs,
        xmlnsAttrib = false ? ' xmlns:'+namespace+'="'+xmlns+'"'+' xmlns="'+xmlns+'"' : '',
        ns = namespace ? namespace + ':' : '';

    if (Array.isArray(obj)) {
        for (var i=0, item; item=obj[i]; i++) {
          subparts = self.objectToXML(item, name, undefined, undefined, attributes);
          if (i > 0) {
              parts.push(['</',ns,name,'>'].join(''));
              parts.push(['<',ns,name,xmlnsAttrib,attributes.join(''),'>'].join(''));
          }
          parts.push(subparts);
          attributes = [];
        }
        ...
        for (var name in obj) {
            var child = obj[name];
            if (name.charAt(0) === '@') {
                attributes.push(' ' + name.substr(1) + '="' + child + '"');
            }
            else {
              attrs = [];
              subparts = self.objectToXML(child, name, undefined, undefined, attrs);
              parts.push(['<',ns,name,xmlnsAttrib,attrs.join(''),'>'].join(''));
              parts.push(subparts);
              parts.push(['</',ns,name,'>'].join(''));
            }
        }
